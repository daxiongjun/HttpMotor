...
LogFileAccessor.prototype = {
	...
	createDocument : function(options,files){
		let doc_name = options.name , _this = this ;
		if(!doc_name || "string" != typeof doc_name){
			throw "无效的文档名" + doc_name ;
		}
		let _document = new LogDocument(options) ;
		this.addDoc(doc_name,_document) ;
		fs.exists(_this.root + doc_name,function(exists){
			if(!exists){
				fs.mkdirSync(_this.root + doc_name) ;
			}
			else{	
				if(files && files.constructor === Array && files.length){
					files.forEach(function(file){
						...
						let file_entity = new LogFileEntity(file) ;
						_this.createFile(file.name,doc_name) ;
						_document.addFile(file_entity) ;
					}) ;
				}
			}
			_this.docs[doc_name] = _document ;
		}) ;
	} ,
	createFile : function(name,doc_name,content){
		let doc = this.getDoc(doc_name) ;
		let file_entity = new LogFileEntity({
			name : name
		}) ;
		if(!doc){
			throw "无效的文档名" + doc_name ;
		...
		doc.addFile("name",file_entity) ;
	} ,
	appendToDocument : function(file_name,doc_name){
		this.createFile(file_name,doc_name) ;
	} ,
	loadDocument : function(value){
		
	} ,
	loadFile : function(value){
		
	} ,
	_InternaLAccessorLoader : function(name,options){
		if(!name || "string" != typeof name){
			throw "无效的文档/文件名" + name ;
		}
	} ,
...
Log4Njs.prototype = {
	...
	init : function(){
		...
		this.fileInternaLProcessor.init() ;
	} ,
	log : function(log){
