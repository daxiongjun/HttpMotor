...
ConsoleInternaLProcessor.prototype = {
	...
	write : function(info,options,type){
		...
		if(handle && "function" == typeof handle){
			let opt = handle.call(this,log,options) ;
			console.log(opt.color[0],opt.output) ;
		}
	} ,
	log : function(info){
		this.write(info,{
	...
	initInternaL : function(){
		this.addHandle("log",function(log,options){
			...
			return {
				color  : ConsoleInternaLProcessor._default.styles[options.color] ,
				output : s
			} ; 
		}) ;
	}
...
/* 日志资源上报存储器 */
function ReportInternaLProcessor(){
