...
/* 日志档案访问器 */
function LogFileAccessor(options){
	this.docBase = options.docBase ;
	if(!this.docBase || "string" != typeof this.docBase){
		this.docBase = "../../sys_log_base" ;
	}
	this.created_doc_base = false ;
	this.fileMap = {} ;
} ;
LogFileAccessor.prototype = {
	constructor : LogFileAccessor ,
	isExistsCreated : function(){
		return this.created_doc_base ;
	} ,
	init : function(){
		let _this = this ;
		let hasExistsCreatedDocBase = fs.existsSync(this.docBase) ;
		if(!hasExistsCreatedDocBase){
			fs.mkdirSync(this.docBase) ;
			this.created_doc_base = true ;
		}
		else{
			let files = fs.readdirSync(this.docBase) ;
			files.forEach(function(v){  // 处理手动建立目录的情况
				let id = v.split(".")[0] ;
				_this.fileMap[id] = v ;
			}) ;
		}
	...
	addFile : function(name,doc,append_content){
		
	} ,
	appendToFile : function(name,content){
