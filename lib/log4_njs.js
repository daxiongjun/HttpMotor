...
LogMapFilter.prototype = {
	...
	initInternaLFilter : function(){
		...
		this.addFilter("regex",function(data,hook){
			
		}) ;
		this.addFilter("date",function(data,hook){
...
LogManager.prototype = {
	...
	findInternaL : function(value,extra){
		let finded_array = LogManager.InnerLogManagerMapFinder.call(this,value) , filter_array = null ;
		if(
			finded_array 
			&& finded_array.constructor === Array 
			&& finded_array.length
			&& "object" == typeof extra
		){
			let filterExecuter = this.logMapFilter.getFilter()[extra.filterType] ;
			filter_array = filterExecuter.call(this,finded_array) ;
		}
		return filter_array ;
	}
} ;
